...
The robot navigates a maze-like disaster environment using DFS, detects letter and colour victims via Raspberry Pi, deploys rescue kits, avoids hazards, handles obstacles, restarts from checkpoints if stuck, and returns to start for exit bonuses, operating fully autonomously.
...
* Recognize victim identifiers such as letters and colours.
...
* Multi-tier modular structure:
  ...
  * Bottom tier: Arduino, motor drivers, line/colour sensors.
  * Middle tier: Infrared distance sensors.
  * Upper tier: Raspberry Pi 5 with dual cameras.
  * Top tier: Lithium batteries and power banks.
...
* OpenCV-based colour detection (HSV masks).
...
* Refactor software using state machines or behaviour trees for modularity.
